...
- Implementasi observasi: `emergency-exit-shadow.js` membaca bin aktif awal dan terkini dari posisi yang dibuka Sunstrike. Penurunan harga terhitung ≥20% selama lima menit menghasilkan satu event `emergency_exit_shadow` dengan `executed=false`; belum ada auto-close maupun cooldown. Posisi manual tanpa data entry bot tidak bisa dinilai oleh monitor ini.
