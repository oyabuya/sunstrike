...
- Evolves `maxVolatility` and `minFeeActiveTvlRatio` — both keys are valid in config.js (confirmed working)
...
- `lessons.js evolveThresholds()` works correctly — evolves `maxVolatility` and `minFeeActiveTvlRatio`, both valid keys in config.js. Requires minimum 5 closed positions.
