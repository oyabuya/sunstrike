...
const sizing = validateNewPosition({ amountSol: config.management.deployAmountSol,
  solPrice: balance.sol_price, walletUsd: balance.total_usd,
  currentExposureUsd: risk.snapshot?.open_exposure_usd, risk: config.risk });
if (!sizing.pass) blockers.push(sizing.reason);
const requiredSol = Math.max(config.management.minSolToOpen,
  config.management.deployAmountSol + config.management.gasReserve + config.management.binArrayRentBuffer);
if (!Number.isFinite(balance.sol) || balance.sol < requiredSol) blockers.push(`Liquid SOL must be at least ${requiredSol}`);
...
console.log(JSON.stringify({ at: new Date().toISOString(), ready_for_entry: blockers.length === 0,
  ...
  deposit: { side: 'SOL only', strategy: config.strategy.strategy, sol: config.management.deployAmountSol },
  liquid_sol: balance.sol ?? null, required_sol: requiredSol,
