...
const INTENT_TOOLS = {
 deploy: new Set(["get_top_candidates", "get_active_bin", "get_pool_memory", "check_smart_wallets_on_pool", "get_token_holders", "get_token_narrative", "get_token_info", "search_pools", "get_wallet_balance", "get_my_positions", "add_pool_note", "get_rugcheck_report", "get_dexscreener_pairs"]),
  ...
  screen:      new Set(["get_top_candidates", "get_token_holders", "get_token_narrative", "get_token_info", "search_pools", "check_smart_wallets_on_pool", "get_pool_detail", "get_my_positions", "discover_pools", "get_trending_tokens", "get_rugcheck_report", "get_dexscreener_pairs"]),
  ...
  trending:    new Set(["get_trending_tokens", "get_token_info", "get_rugcheck_report", "get_dexscreener_pairs"]),
};
...
const INTENT_PATTERNS = [
  ...
  { intent: "trending",    re: /\b(trending|hot|what.?s trending|narrative|sector|pumping|viral|trending now|top token)\b/i },
  { intent: "screen",      re: /\b(security|rug.?check|audit|safe|is it safe|honeypot|scam|legit)\b/i },
];
...
import { isAllowedAutonomousManagementTool } from "./position-ownership.js";
...
const DEFAULT_MODEL = process.env.LLM_MODEL || "openai/gpt-4.1-mini";
...
export async function agentLoop(goal, maxSteps = config.llm.maxSteps, sessionHistory = [], agentType = "GENERAL", model = null, maxOutputTokens = null, options = {}) {
  const { requireTool = false, interactive = false, onToolStart = null, onToolFinish = null, allowedDeployPoolAddresses = null, allowedManagementPositionAddresses = null } = options;
  ...
  for (let step = 0; step < maxSteps; step++) {
    ...
    try {
      ...
      const toolResults = await Promise.all(msg.tool_calls.map(async (toolCall) => {
        ...
        if (!isAllowedAutonomousManagementTool(functionName, functionArgs, allowedManagementPositionAddresses)) {
          const reason = "Autonomous management may only close or claim its approved bot positions.";
          log("agent", `Blocked autonomous ${functionName} outside approved positions`);
          await onToolFinish?.({ name: functionName, args: functionArgs, result: { blocked: true, reason }, success: false, step });
          return { role: "tool", tool_call_id: toolCall.id, content: JSON.stringify({ blocked: true, reason }) };
        }
