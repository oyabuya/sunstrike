...
- Permintaan UX Telegram: audit menunjukkan hanya `/briefing`, `/positions`, `/close`, `/set`, dan mode punya handler langsung; `/candidates` dan `/evolve` sebelumnya hanya berfungsi di terminal. Patch lokal menambah menu Telegram, `/help`/`/start`, alias `/candidat` dan `/refresh` untuk kandidat baca-saja, `/check`/`/status` untuk kondisi bot, `/thresholds`, dan `/evolve` dengan syarat 5 posisi tertutup. Slash tidak dikenal mendapat panduan. `/close` lewat executor agar post-close swap tetap dijalankan. Belum diuji/deploy pada catatan ini.
