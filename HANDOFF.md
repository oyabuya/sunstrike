...
- Permintaan terbaru `/check`: tampilkan seluruh token bersaldo positif pada wallet, bukan hanya SOL. Patch lokal menambah daftar token dan nilai USD per aset dari snapshot Helius yang sama, nilai USD hilang ditandai jelas, pesan panjang dibagi agar tidak terpotong Telegram. Berlaku juga untuk `/status` karena keduanya memakai handler sama. Belum dideploy saat catatan ini.
