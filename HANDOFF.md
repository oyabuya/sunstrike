...
- Siklus DRY_RUN penuh berikutnya membuktikan klaim kandidat tersebut salah: agent mencoba `X7-SOL`, tetapi preflight menolak `pool not found`; tidak ada transaksi. Pool `7XeEPR9o...` dari Pool Discovery 2h mendapat 404 di API detail Meteora dan `getAccountInfo` Helius mengembalikan null. Tiga pool DLMM kontrol dari API yang sama mendapat detail HTTP 200 dan akun RPC dimiliki program DLMM, sehingga masalah spesifik pada entri discovery. Upstream Yunus-0x juga memakai Pool Discovery dan memverifikasi lewat SDK `DLMM.create` sebelum deploy; fallback Sunstrike menampilkan entri invalid terlalu dini. Patch lokal menambahkan verifikasi akun pool+owner DLMM sebelum pool menjadi kandidat. Status LIVE belum diklaim siap memasang LP; perlu commit/deploy/verifikasi patch ini.
