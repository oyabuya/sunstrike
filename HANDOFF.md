...
- Persiapan Telegram: `.env` chat ID kini diutamakan atas nilai lama `user-config.json`; `scripts/telegram-check.js` memvalidasi token dan menampilkan chat/user ID tanpa mencetak token, dengan `--send-test` untuk pesan uji eksplisit. Pemilik masih perlu membuat bot via BotFather, menaruh token di VPS, dan mengirim `/start`.
